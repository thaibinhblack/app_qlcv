...
export default {
    ...
    methods: {
        api_loai_du_an()
        {
            ...
            this.axios.post(this.$store.state.config.API_URL + 'du-an?api_token='+this.$session.get('token'),loai_du_an).then((response) => {
        ...
        updateLoaiDuAn()
        {
            ...
            loai_du_an.append("P_trang_thai", this.loai_du_an.trang_thai);
            const app = this;
            this.axios.post(this.$store.state.config.API_URL + 'du-an/'+this.loai_du_an.id_loai_du_an+'?api_token='+this.$session.get('token'),loai_du_an).then((response) => {
                app.loai_du_an = {
                    trang_thai: 1
                }
