...
<form @submit.prevent="api_du_an()" class="form-rule" style="padding: 15px;">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-9">
            <multiselect v-model="selected_kh" :options="khach_hang" label="username_nd" track-by="id_nd"  style="z-index:9999"></multiselect>
        </div>
    </div>
    <div class="form-group row">
...
export default {
    ...
    methods: {
        api_kh()
        {
            ...
            this.axios.post(this.$store.state.config.API_URL + 'customers?api_token='+this.$session.get('token'),kh).then((response) => {
