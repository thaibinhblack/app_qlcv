<template>
    <form>
        ...
        <div class="form-group row">
            <div class="col-sm-9">
                <button v-if="getAction_user.them == '2.1'" type="button" :disabled="update"  class="btn btn-primary btn-form" @click="api_resignter_user()">Thêm</button>
                <button v-if="getAction_user.sua == '2.2'" type="button" :disabled="!update" class="btn btn-warning btn-form" @click="api_update_user()">Cập nhật</button>
                <button v-if="getAction_user.sua == '2.2'" type="submit" :disabled="!update" @click="$emit('clear',false)" class="btn btn-success btn-form">Clear</button>
            </div>
...
export default {
    ...
    methods:
    {
        ...
        api_update_user()
        {
            ...
            user.append("NGAY_SINH",this.user.ngay_sinh_nd.getDate() +'-'+this.user.ngay_sinh_nd.getMonth() + '-' + this.user.ngay_sinh_nd.getFullYear())
            const app = this;
            this.axios.post(this.$store.state.config.API_URL + 'user/'+this.user.id_nd+'?api_token='+this.$session.get('token'),user).then((response) => {
                app.$emit('createUser',true)
                app.$buefy.notification.open({
                    duration: 5000,
                    message: response.data.message,
        ...
        FilterGroup()
        {
            ...
            this.axios.get(this.$store.state.config.API_URL + 'group_user/'+this.trung_tam_selected+'?api_token='+this.$session.get('token')).then((response) => {
                this.$emit('updateGroups',response.data)
            })
        }
    }
