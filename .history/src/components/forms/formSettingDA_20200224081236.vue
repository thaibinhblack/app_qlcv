 ...
 <form class="form-rule" style="padding: 15px;">
    <div class="row" v-for="(setting,index) in settings" :key="index" style="margin: 10px 0;">
        ...
        <div class="col-sm-12 col-md-2">
            <b-button :icon-left="setting.HIDDEN_ATTRIBUTE == false ? 'delete' : 'check'" @click="setting.HIDDEN_ATTRIBUTE = !setting.HIDDEN_ATTRIBUTE"></b-button>
        </div>
    </div>
    <div class="row">
...
export default {
    ...
    methods: {
        ...
        createSetting()
        {
            if(Object.entries(this.du_an_edit).length > 0)
            {
                this.$store.dispatch("createSettingDuAn",{
                    id_du_an: this.du_an_edit.id_du_an,
