    ...
    <div class="row">
        <div class="col-sm-12 col-md-6">
            ...
            <div class="form-group row">
                ...
                <label for="inputPassword3" class="col-sm-4 col-form-label" >Màu loại công việc</label>
                <div class="col-sm-8">
                <b-input type="text" maxlength="50" placeholder="Màu loại công việc" v-model="colors.hex" @input="active = !active"></b-input>

                </div>
                {{active}}
                <div class="group-color card" :class="{active: active}">
                    <chrome-picker :value="colors" @input="updateColor"></chrome-picker>
                    <button type="button" @click="active =!active"  class="btn btn-primary btn-form" style="margin-top:10px;margin-left: 0;">Xác nhận</button>
...
export default {
    ...
    methods:
    {
        api_loai_cv()
        {
            ...
            form_loai_cv.append("P_COLOR",this.colors.hex)
            const app = this;
            this.axios.post(this.$store.state.config.API_URL + 'loai-cv?api_token='+this.$cookies.get('token'),form_loai_cv).then((response) => {
                ...
                {
                    var type =  response.data.success == true ? 'is-success' : 'is-danger'
...
<style scoped>
.group-color {position: absolute;top: 0;right: 0;display: none;}
.group-color.active {display: block;}
</style>