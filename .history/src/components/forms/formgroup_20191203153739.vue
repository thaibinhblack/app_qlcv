<template>
    <form class="form-rule" >
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8">
            <b-input maxlength="50" v-model="group.ten_nhom" type="text"  placeholder="Tên phòng"></b-input>
            </div>
        </div>
         <div class="form-group row">
...
export default {
    ...
    methods: {
        ...
        update_group()
        {
            ...
            group.append("TRANG_THAI_NHOM",this.group.trang_thai_nhom)
            const app = this;
            this.axios.post(this.$store.state.config.API_URL + 'group_user/'+this.group.id_nhom+'?api_token='+this.$cookies.get('token'),group).then((response) => {
