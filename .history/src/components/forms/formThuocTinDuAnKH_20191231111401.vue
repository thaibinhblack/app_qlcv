...
export default {
    ...
    methods:
    {
        api_create_thongtin()
        {
            ...
            {
                const form_data= new FormData();
                form_data.append("P_ID_DU_AN_KH",this.tt_edit.id_du_an_kh)
                form_data.append("P_TEN_THONGTIN",this.tt_edit.ten_thongtin)
                form_data.append("P_NOI_DUNG_THONGTIN",this.tt_edit.noi_dung_thongtin)
                this.$store.dispatch('updateThongTin',this.tt_edit);
                // console.log(this.du_an_kh)
                // // var app = this;
                // this.axios.post(this.$store.state.config.API_URL + 'thong-tin-duan-kh/'+this.tt_edit.id_thongtin+'?api_token='+this.$cookies.get('token'),form_data).then((response) => {
