...
<form @submit.prevent="api_create_thongtin()" class="form-rule" style="padding: 15px;">
    <div class="row" v-if="Object.entries(getListThongTin).length > 0"> 
     ...
     <div class="form-group row" >
         <div class="form-group col-sm-12 col-md-6 row" v-if="Object.entries(this.tt_edit).length === 0">
...
export default {
    ...
    data()
    {
        return {
            attribute: {
                ten_thongtin: "",
                noi_dung_thongtin: "",
                id_du_an_kh: this.du_an_kh.id_du_an_kh
            },
            tt_edit: {
    ...
    methods:
    {
        ...
        {
            var app = this;
            const result  = this.$store.dispatch('deleteThongTin',this.tt_edit);
                if(result == false)
                {
                    app.$buefy.notification.open({
                        duration: 1500,
