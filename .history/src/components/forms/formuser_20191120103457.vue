    ...
    <form>
        ...
        <div class="form-group row">
            <div class="col-sm-9">
                ...
                <button type="submit" :disabled="!update" class="btn btn-warning btn-form" @click="api_update_user()">Cập nhật</button>
...
export default {
    ...
    methods:
    {
        ...
        api_update_user()
        {
            ...
            user.append("NGAY_SINH",this.user.ngay_sinh_nd.getDate() +'-'+this.user.ngay_sinh_nd.getMonth() + '-' + this.user.ngay_sinh_nd.getFullYear())
            const app = this;
            this.axios.post(this.$store.state.config.API_URL + 'resignter/'+this.user.id_nd+'?api_token='+this.$session.get('token'),user).then((response) => {
                app.$emit('createUser',true)
                 app.$buefy.notification.open({
                    duration: 5000,
                    message: response.data.message,
