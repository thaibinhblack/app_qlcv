...
export default {
    ...
    methods: {
        api_du_an()
        {
            ...
            this.axios.post(this.$store.state.config.API_URL + 'loai-du-an?api_token='+this.$session.get('token'),du_an).then((response) => {
        ...
        updateLoaiDuAn()
        {
            ...
            du_an.append("P_TRANG_THAI_DU_AN", this.du_an.trang_thai_du_an);
            const app = this;
            this.axios.post(this.$store.state.config.API_URL + 'loai-du-an/'+this.du_an.id_du_an+'?api_token='+this.$session.get('token'),du_an).then((response) => {
                app.du_an = {
                    trang_thai_du_an: 1
                }
