...
<b-tabs>
    ...
    <b-tab-item label="Nội dung tiến độ">
        <div class="form-group row" v-for="(cv,index) in congviec" :key="index">
            <div class="col-sm-12 col-md-12 col-form-label">{{cv.display_name}}</div>
...
export default {
    ...
    methods: {
        api_bao_cao()
        {
            ...
            baocao.append("P_TIEN_DO",this.my_info.tien_do)
        ...
        api_get_baocao()
        {
            this.axios.get(this.$store.state.config.API_URL + 'cong-viec/'+this.cong_viec_edit.id_cv_da+'/baocao?api_token='+this.$cookies.get('token'))
            .then((response) => {
