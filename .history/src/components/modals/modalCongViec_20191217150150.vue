...
export default {
    ...
    computed:{
      ...
      nguoi_nhap()
      {
        return this.users.filter((value,index,array) => {
          return array[index].id_nd == this.cong_viec.nguoi_nhap
        })[0].display_name
      },
      ngay_tiep_nhan()
      {
    ...
    methods:
    {
      ...
      api_users_giaoviec()
      {
        this.axios.get(this.$store.state.config.API_URL + 'user-giaoviec?api_token='+this.$cookies.get('token')+'&id_du_an='+this.selected_du_an_kh[0].id_du_an).then((response) => {
      ...
      api_users()
      {
        ...
        chose_lcv(selectedOption, id)
        {
          if(selectedOption[0])
          {
            // alert('Không được chọn cha')
            this.selected_loai_cv =null
          }
          console.log('chose',selectedOption, id)
        }
    },
    created()
    {
