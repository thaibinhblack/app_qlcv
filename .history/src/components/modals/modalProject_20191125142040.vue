...
<form style="padding: 15px;">
    ...
    <div class="form-group row">
        <label for="inputPassword3" class="col-sm-3 col-form-label" >Khách hàng {{ selected_kh}}</label>
        <div class="col-sm-9">
             <multiselect v-model="selected_kh" :options="khach_hang"  label="ten_kh" trackBy="id_khach_hang"></multiselect>
        </div>
    </div>
    <div class="form-group row">
        ...
        api_khach_hang()
        {
            this.axios.get(this.$store.state.config.API_URL + 'customers?api_token='+this.$session.get('token')).then((response) => {{
                ...
                this.selected_kh = response.data.results.filter((value,index,array) => {
                    return array[index].id_khach_hang == this.selected_project.id_khach_hang
                })[0]
            }})
        },
        close()
