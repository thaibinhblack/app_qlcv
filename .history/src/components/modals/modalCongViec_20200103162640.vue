...
export default {
    ...
    computed:{
      ...mapGetters(["getUser", "getTaskEdit", "GROUP_LCV", "LIST_DUAN", "LIST_DUAN_KH", "GROUP_LCV", "LIST_USER_GIAOVIEC", "LIST_USER"]),
      nguoi_tham_dinh()
      {
    ...
    created()
    {
        ...
        if(this.getTaskEdit.id_cv_da)
        {
          ...
          this.selected_loai_cv = this.GROUP_LCV.filter((value,index,array) => {
            return array[index].id_loai_cv == this.getTaskEdit.id_loai_cv
          })[0]
          console.log('loại công việc',this.selected_loai_cv)
