                    ...
                    <div class="col-sm-8 col-md-9">
                        ...
                        <div class="form-group row">
                            <div class="col-sm-9">
                                ...
                                <button @click="update_quyen_thanh_vien()" type="button" class="btn btn-primary btn-form" >Cập nhật</button>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        update_quyen_thanh_vien()
        {
            const fomr_quyen = new FormData();
            fomr_quyen.append('id_nd',this.nhanvien_selected)
            fomr_quyen.append("quyen",this.quyen_duan)
            fomr_quyen.append("id_du_an_kh",this.selected_project)
            this.axios.post(this.$store.state.config.API_URL + 'cap-nhat-quyen-thanhvien?api_token='+this.$cookies.get('api_token'),fomr_quyen).then((response) => {
        ...
        api_du_an_kh(id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'du-an-kh/'+id+'?api_token='+this.$cookies.get('token')).then((response) => {
        ...
        api_update_du_an()
        {
            ...
            this.axios.post(this.$store.state.config.API_URL + 'nhanvien-da?api_token='+this.$cookies.get('token'),form_nhanvien).then((response) => {
                console.log(response.data)
            })
        ...
        {
            this.axios.get(this.$store.state.config.API_URL + 'quyen-nhanvien-da?api_token='+this.$cookies.get('token')+'&id_nd='+id+'&id_du_an_kh='+this.selected_project)
            .then((response) =>{
                this.quyen_duan = [response.data[0].q_xem,response.data[0].q_them,response.data[0].q_sua,response.data[0].q_xoa,response.data[0].q_xuat]
            })
        }
