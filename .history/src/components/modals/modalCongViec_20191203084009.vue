        ...
        <div class="form-group row">        
         ...
         <div class="form-group row">        
          ...
          <div class="col-sm-8">
               ...
               <b-input disabled type="text" v-model="my_info.username_nd" required placeholder="Người nhập công việc" ></b-input>
          </div>
        </div>
      </div>
      <div class="col-sm-12 col-md-6">
      ...
      api_users()
      {
        ...
        api_cong_viec(P_ACTION)
        {
          ...
          this.axios.post(this.$store.state.config.API_URL + 'cong-viec?api_token='+this.$session.get('token'),cong_viec).then((response) => {
              ...
              const content = this.my_info.username_nd + ' vừa tạo công việc ' + this.cong_viec.ten_cv +' Nội dung phần mềm: '+ this.cong_viec.noi_dung_cv
        ...
        api_capnhat_congviec()
        {
            ...
            cong_viec.append("P_NGAY_CAM_KET",this.cong_viec.ngay_cam_ket.getFullYear() + '-' + ( this.cong_viec.ngay_cam_ket.getMonth() + 1) + '-' + this.cong_viec.ngay_cam_ket.getDate())
            this.axios.post(this.$store.state.config.API_URL + 'capnhat_congviec/'+this.cong_viec_edit.id_cv_da+'?api_token='+this.$session.get('token'),cong_viec).then((response) => {
              app.$buefy.notification.open({
                  duration: 1500,
