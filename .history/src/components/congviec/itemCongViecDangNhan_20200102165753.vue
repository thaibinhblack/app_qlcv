...
<div class="col-4 col-md-4 col-sm-6 col-lg-3 col-xl-3 item-group-task">
            <div class="card card-task" :class="{not: zoom == 'col-12'}">
                <div class="card-title">
                    <h3 style="line-height:36px">Công việc đang nhận ({{getCongViecByStatus2.length}}) <b-button @click="zoomLayout()" class="btn-zoom" icon-right="plus"></b-button></h3>
                </div>
                 <div class="search-task">
                     <b-input v-model="search_getCongViecByStatus2" type="text" placeholder="Tìm kiếm công việc" ></b-input>
                </div>
                <draggable class="list-group" :list="getCongViecByStatus2" group="people"  @change="update_task_2">
                  
                    <div
                        class="list-group-item"
                        v-for="(element, index) in getCongViecByStatus2"
                        :key="index"
...
export default {
    ...
    computed: {
        ...mapGetters(["getCongViecByStatus2"])
    }
}
</script>