    ...
    methods: {
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                const functions_check_rule = []
                var object =[]
                for (let index = 0; index < this.functions.length; index++) {
                    functions_check_rule.push([]) 
                }
                for (let index = 0; index < response.data.results.length; index++) {
                    ...
                    functions_check_rule[response.data.results[index].ID_FUNCTION -1 ] = object
                    object= []
                    
                }
                // this.functions_check_rule  = functions_check_rules
