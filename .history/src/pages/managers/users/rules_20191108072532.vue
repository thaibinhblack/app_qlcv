            ...
            <div class="col-md-10">
                ...
                <div class="card card-rule">
                    ...
                    <div class="row" style="margin:7px 0" v-for="(function_rule, index) in functions" :key="index">
                        <div class="col-sm-2">{{function_rule.NAME_FUNCTION}}</div>
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                response.data.forEach((detail) => {
                    this.functions_check_rule.push(detail.ID_FUNCTION +'.' + detail.FUNCTION_VIEW)
                    this.functions_check_rule.push(detail.ID_FUNCTION +'.' + detail.FCUNCTION_CREATE)
                    this.functions_check_rule.push(detail.ID_FUNCTION +'.' + detail.FUNCTION_EDIT)
                    this.functions_check_rule.push(detail.ID_FUNCTION +'.' + detail.FUNCTION_DELETE)
                })
            })
        }
    },
