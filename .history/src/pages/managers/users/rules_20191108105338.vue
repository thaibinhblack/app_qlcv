                ...
                <div class="card card-rule">
                    ...
                    <div class="row" style="margin:7px 0" v-for="(function_rule, index) in functions" :key="index">
                        ...
                        <div class="col-sm-2 col-md-2 text-rule"> 
                            <div class="field" >
                                <b-checkbox v-model="functions_check_rule[index].VIEW"  :native-value="function_rule.ID_FUNCTION +'.'+ 1"></b-checkbox>
                            </div>
                        </div>
                        <div class="col-sm-2 col-md-2 text-rule">
                             <div class="field">
                                <b-checkbox v-model="functions_check_rule[index].CREATE" :native-value="function_rule.ID_FUNCTION +'.' + 2"></b-checkbox>
                            </div>
                        </div>
                        <div class="col-sm-2 col-md-2 text-rule">
                             <div class="field">
                                <b-checkbox v-model="functions_check_rule[index].EDIT" :native-value="function_rule.ID_FUNCTION +'.' + 3"></b-checkbox>
                            </div>
                        </div>
                        <div class="col-sm-2 col-md-2 text-rule">
                             <div class="field">
                                <b-checkbox v-model="functions_check_rule[index].DELETE" :native-value="function_rule.ID_FUNCTION +'.' + 4"></b-checkbox>
                            </div>
                        </div>  
                    </div>
                    <div class="row">
...
export default {
    ...
    methods: {
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                for (let index = 0; index < response.data.results.length; index++) {
                    ...
                    object["VIEW"] = response.data.results[index].FUNCTION_VIEW
                    object["CREATE"]= response.data.results[index].FUNCTION_CREATE
                    object["EDIT"] = response.data.results[index].FUNCTION_EDIT
                    object["DELETE"] = response.data.results[index].FUNCTION_DELETE
                   
                    this.functions_check_rule[response.data.results[index].ID_FUNCTION -1 ] = object
                     console.log('lenght',response.data.results[index].ID_FUNCTION -1 , this.functions_check_rule)
                    object= []
        ...
        updateFunctionRule()
        {
            console.log(this.functions_check_rule.length)
            this.functions_check_rule.forEach((check) => {
                    ...
                    if(check.length >= 1)
                    {
                        ...
                        check.forEach((value) => {
                            value_function.push(value)
                        })
                        const function_view = (check[0]) ?  check[0] : 'null'
                        const function_edit = (check[2]) ? check[2] : 'null'
                        const function_create = (check[1]) ? check[1] : 'null'
                        const function_delete = (check[3]) ?  check[3] : 'null'
                        ...
                        console.log('test',value_function)
                ...
                // else if(check.length >= 1 && check.length < 4)
                // {
                //     const id_function = check[0].slice(0,1)
                //     const function_view = null
                //     const function_edit = null
                //     const function_create = null
                //     const function_delete = null
                //     const data = new FormData()
                //     check.forEach((value) => {
                //         console.log('test', value.slice(2,3))
                //         if(value.slice(2,3) == 1)
                //         {
                //             function_view = value
                //         }
                //         else if(value.slice(2,3) == 2)
                //         {
                //             function_edit = value
                //         }
                //         else  if(value.slice(2,3) == 3)
                //         {
                //             function_create = value
                //         }
                //         else  if(value.slice(2,3) == 4)
                //         {
                ...
                //     data.append("FUNCTION_VIEW",function_view)
                //     data.append("FUNCTION_CREATE",function_create)
                //     data.append("FUNCTION_EDIT",function_edit)
                //     data.append("FUNCTION_DELETE",function_delete)
