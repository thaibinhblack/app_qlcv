    ...
    methods: {
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                console.log(this.functions.length - response.data.results.length)
                if(this.functions.length > response.data.results.length)
                {
                    for (let index = 1; index < (this.functions.length - response.data.results.length); index++) {
