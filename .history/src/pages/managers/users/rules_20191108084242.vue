    ...
    methods: {
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                console.log(response.data.results.length, this.functions.length)
                if(response.data.results.length > 0)
                {
                    this.functions_check_rule.push(object)
                    for (let index = response.data.results.length  ; index < (this.functions.length - response.data.results.length); index++) {
                        const object_function = []
                        console.log('test',index)
                        object_function.push(this.functions[index+1].ID_FUNCTION + '.' +0, this.functions[index+1].ID_FUNCTION + '.' + 0,
                        this.functions[index+1].ID_FUNCTION + '.' + 0, this.functions[index+1].ID_FUNCTION + '.' + 0)
