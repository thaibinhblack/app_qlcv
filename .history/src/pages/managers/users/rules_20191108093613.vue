                ...
                <div class="card card-rule">
                    ...
                    <div class="row" style="margin:7px 0" v-for="(function_rule, index) in functions" :key="index">
                        ...
                        <div class="col-sm-2 col-md-2 text-rule"> 
                            <div class="field" >
                                <b-checkbox v-model="functions_check_rule"  :native-value="function_rule.ID_FUNCTION +'.'+ 1"></b-checkbox>
                            </div>
                        </div>
                        <div class="col-sm-2 col-md-2 text-rule">
                             <div class="field">
                                <b-checkbox v-model="functions_check_rule" :native-value="function_rule.ID_FUNCTION +'.' + 2"></b-checkbox>
                            </div>
                        </div>
                        <div class="col-sm-2 col-md-2 text-rule">
                             <div class="field">
                                <b-checkbox v-model="functions_check_rule" :native-value="function_rule.ID_FUNCTION +'.' + 3"></b-checkbox>
                            </div>
                        </div>
                        <div class="col-sm-2 col-md-2 text-rule">
                             <div class="field">
                                <b-checkbox v-model="functions_check_rule" :native-value="function_rule.ID_FUNCTION +'.' + 4"></b-checkbox>
                            </div>
                        </div>  
                    </div>
                    <div class="row">
    ...
    methods: {
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                var object =[]
                console.log('lenght',response.data.results.length)
                for (let index = 0; index < response.data.results.length; index++) {
                    this.functions_check_rule.push(response.data.results[index].FUNCTION_VIEW)
                    this.functions_check_rule.push(response.data.results[index].FCUNCTION_CREATE)
                    this.functions_check_rule.push(response.data.results[index].FUNCTION_EDIT)
                    this.functions_check_rule.push(response.data.results[index].FUNCTION_DELETE)
               ...
               if(check.length == 4)
                    {
                        ...
                        const function_view = check[0]
                        const function_edit = check[2]
                        const function_create = check[1]
                        const function_delete = check[3]
                        ...
                        // console.log('test',id_function,function_view,function_edit,function_create),function_delete
