    ...
    methods: {
        ...
        api_function_rule($id)
        {
            this.axios.get(this.$store.state.config.API_URL + 'function_rule/'+$id).then((response) => {
                ...
                console.log((this.this.functions.length - response.data.results.length)
                if(this.this.functions.length > response.data.results.length)
                {
                    for (let index = 0; index < (this.this.functions.length - response.data.results.length); index++) {
