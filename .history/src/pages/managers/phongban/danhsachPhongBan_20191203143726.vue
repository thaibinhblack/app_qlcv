    ...
    <div class="row">
        <div class="col-sm-12 col-md-8 col-lg-7" style="margin:auto">
            <div class="card">
                ...
                @rollback="updatePhongBan = $event" :update="update"/>
            </div>
        </div>
    </div>
...
export default {
        ...
        create_phong_ban(newVal)
        {
            if(newVal == true)
            {
                ...
                this.source.localdata = {
                    ...
                    { name: 'trang_thai_nhom', type: 'number' },
                    { name: 'created_at', type: 'date'},
                    { name: 'id_tt', type: 'number'}
                    ],
                    url: this.$store.state.config.API_URL + 'group_user?api_token='+this.$cookies.get('token'),
    ...
    methods: {
        ...
        onRowclick: function (event) {
           ...
           if(this.updatePhongBan == false)
           {
                this.updatePhongBan = true
           ...
           else
           {
               this.$buefy.notification.open({
                    duration: 1500,
                    message: `Bạn chưa cập nhật, xóa hoặc clear để thực hiện lại`,
