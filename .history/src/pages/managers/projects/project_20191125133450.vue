...
<div id="page-project">
              ...
              <div class="item-card" v-for="i in 5" :key="i">
               ...
               <div class="item-card">
                <b-navbar-item  class="item-user item-user-add">
                    ...
                    <b-button icon-left="plus" @click="isActiveModalProject = !isActiveModalProjects"></b-button>
  ...
  <b-modal :active.sync="isActiveModalProject" :width="'900px'" :can-cancel="false">
    ...
    <modal-project :selected_project="selected_project" :du_an="du_an" :du_an_selected="du_an_selected" />
  </b-modal>
</div>
</template>
<script>
  ...
  methods: {
    ...
    api_cong_viec(id)
    {
      ...
      this.axios.get(this.$store.state.config.API_URL + 'cong-viec/'+id+'?api_token='+this.$session.get('token')).then((response) => {
        ...
        app.du_an_selected = response.data.filter((value,index,array) => {
            return array[index].id_du_an == newVal
        })[0]
