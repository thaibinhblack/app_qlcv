...
export default {
  ...
  methods: {
    ...
    update_task_2: function(evt) {
       if(evt.added)
       {
         this.axios.post(this.$store.state.config.API_URL + 'cong-viec/'+evt.added.element.id_cv_da+'?api_token='+this.$session.get('token')+'&trang_thai='+ evt.added.trang_thai).then((response) => {
