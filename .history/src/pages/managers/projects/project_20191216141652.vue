...
<div style="min-height:100%;">
  ...
  <div id="page-project" class="row" style="min-height:100%;">
    <div class="col-md-12 col-lg-12">
      ...
      <b-navbar class="menu-left" style="height:20px;padding:0">
          <template slot="start">
              ...
              <div class="item-card" v-for="(nv,i) in nhanvien_thamgia" :key="i">
                ...
                <div class="card card-info-user">
                      <div class="row">
                        ...
                        <div class="col-sm-8">
                          <p>{{nv.username_nd}}</p>
                          <p>Quyền: {{nv.q_xem == selected_project+'.1' ? '#xem, ': ''}}  {{nv.q_them == selected_project+'.2' ? '#them, ': ''}}
                             {{nv.q_sua == selected_project+'.3' ? '#sua, ': ''}} {{nv.q_xoa == selected_project+'.4' ? '#xoa, ': ''}}  {{nv.q_xuat == selected_project+'.5' ? '#xuat': ''}}
                          </p>
                        </div>
                      </div>
                  </div>
  ...
  <b-modal :active.sync="isActiveModalProject" :width="'900px'" :can-cancel="false">
    ...
    <modal-project :nhanvien_thamgia="nhanvien_thamgia" :selected_project="selected_project" :du_an_selected="du_an_selected" @close="isActiveModalProject = $event" />
  </b-modal>
  <b-modal :active.sync="isActiveModalGiaHan" :width="'600px'" :can-cancel="false">
    <p class="background" style="padding:15px">Danh mục gia hạn thời gian công việc</p>
    <modal-gia-han :cong_viec_edit="cong_viec_edit" @close="isActiveModalGiaHan = $event" />
  </b-modal>
</div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    api_cong_viec(id)
    {
      ...
      this.axios.get(this.$store.state.config.API_URL + 'cong-viec/'+id+'?api_token='+this.$session.get('token')).then((response) => {
        ...
        // })[0]
        // response.data = this.cong_viec = response.data.filter((value,index,array) => {
        //   return array[index].nguoi_nhan_viec == this.getUser.id_nd || array[index].nguoi_giao_viec == this.getUser.id_nd
        // })
    ...
    api_nhanvien_thamgia(id){
      this.axios.get(this.$store.state.config.API_URL + 'nhanvien-da?api_token='+this.$session.get('token')+'&id_du_an_kh='+id).then((response) => {
        this.nhanvien_thamgia = response.data
      })
    }
  },
  created()
  {
    ...
    this.api_nhanvien_thamgia(this.$route.query.id_du_an_kh)
    this.api_users()
...
#page-project {background-image: url('../../../assets/images/banner-project.jpg');height: 100%;background-size: cover;background-repeat: no-repeat;background-size: 100% 100%;}
