            ...
            <div class="header header-datalist">
                ...
                <ul class="list-action-data top">
                    <li v-if="INFO_USER.id_rule > 0">
                      <b-button :disabled="checkedRows.length > 0 ? false : true" class="btn btn-add height43" @click="gui_tham_dinh()" >{{filter_tham_dinh == 0 ? 'Gửi thẩm định' : 'Hủy thẩm định'}}</b-button></li>
                    <li v-if="INFO_USER.id_rule > 0">
                      <b-button :disabled="checkedRows.length > 0 ? false : true" class="btn btn-add height43" @click="tham_dinh()" >Thẩm định</b-button></li>
                    <!-- {{filter}} -->
                    <li>
                      <b-field>
...
export default {
    ...
    computed:{
        ...mapGetters([ "GET_SETTING", "setting_modal", "total_time_cho_tham_dinh", "INFO_USER", "LIST_CONG_VIEC_CTD", "LIST_USER",
         "GROUP_LCV", "LIST_DUAN_KH", "LIST_DUAN"])
    },
    watch:
    {
