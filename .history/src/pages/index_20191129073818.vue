...
export default {
    ...
    methods:{
        ...mapActions(["commitFunctioUser", "commitUser"]),
        api_function()
        {
        ...
        api_get_token(){
            this.axios.get(this.$store.state.config.API_URL + 'token?api_token='+this.$session.get('token')).then((response) => {
                ...
                this.axios.get(this.$store.state.config.API_URL + 'function_user?api_token='+this.$session.get('token')+'&ID_ND='+response.data.id_nd).then((response) => {
                    console.log(response.data)
                })
