    ...
    sendThamDinh({commit},array_cv)
    {
        ...
        array_cv.list_cv.forEach((cv) => {
            ...
            arr_user.forEach((user) => {
                console.log(user,ccv.display_name)
                if(cv.display_name == user)
                {
                    $check = 1;
                    return;
                }
        ...
        //                 'text':   store.state.module_users.info_user.display_name + ' YÊU CẦU THẨM ĐINH CÔNG VIỆC!'
        //             });
        //         })
        //     .catch((err) => {
        //         reject(err)
