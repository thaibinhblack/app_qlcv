...
export default {
    ...
    fetchCongViecPhanCong({commit},filter)
    {
        return new Promise((resolve,reject) => 
        {
            axios.get('/api/cong-viec?api_token='+axios.defaults.params.api_token
                +'&time_start='+filter.time_start+'&time_end='
                +filter.time_end+'&P_ID_LOAI_CV='+filter.P_ID_LOAI_CV
                +'&id_du_an='+filter.id_du_an).then((response) => {
