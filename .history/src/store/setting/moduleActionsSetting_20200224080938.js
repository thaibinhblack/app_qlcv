...
export default {
    ...
    updateValueSetting({commit},du_an)
    {
        // console.log(du_an)
        return new Promise((resolve,reject) => {
            const form_value_setting = new FormData();
            form_value_setting.append("P_VALUE_ATTRIBUTE", JSON.stringify(du_an.value_setting))
            axios.post('/api/setting-value-du-an-kh/'+du_an.id_du_an_kh+'/?api_token='+axios.defaults.params.api_token,form_value_setting)
            .then((response) => {
                resolve(response.data)
            })
            .catch((err) => {
