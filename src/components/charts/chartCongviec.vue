...
import { mapGetters } from 'vuex';
    export default {
                ...
                seriesGroups: [
                    {
                        ...
                        type: 'line',
                        series: [
                            { dataField: 'sl', displayText: 'Biểu đồ công việc', opacity: 0.7, radius: 2, lineWidth: 2, symbolType: 'circle' },
        ...
        computed:{
            ...mapGetters(["getDataset_congviec"])
        },
        watch:
        {
            getDataset_congviec(val)
            {
                this.$refs.myChart.source = val
                this.$refs.myChart.update()
                console.log('watch',this.$refs.myChart)
            },
            dataset(val)
            {
                this.$refs.myChart.source = val
        ...
        mounted()
        {
            ...
            // this.$refs.myChart.source = this.get
             this.$refs.myChart.source = this.getDataset_congviec
            this.$refs.myChart.update()
